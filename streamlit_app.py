...
def kpi_block(latest_actual, first_fore, last_fore, unit=""):
    ...
    col2.metric("Next day forecast", f"{fmt(first_fore)}{unit}", delta=fmt((first_fore-latest_actual),2) if latest_actual==latest_actual else None)
    chg = (last_fore/latest_actual - 1.0)*100 if latest_actual and not math.isclose(latest_actual,0.0) else None
    col3.metric("Change over horizon", (f"{fmt(last_fore)}{unit}"), (f"{fmt(chg,2)}%") if chg is not None else None)

# Load config (ticker presets)
cfg = {}
...
with tab1:
    ...
    if os.path.exists(path):
        ...
        # KPIs from the 1-week forecast for context
        latest_actual = pd.to_numeric(df["Price"], errors="coerce").dropna().iloc[-1] if not df.empty else None
        ...
        for i, h in enumerate(HORIZONS):
            with cols[i]:
                ...
                csv_name = f"{'yahoo' if choice.startswith('Yahoo') else 'worldbank'}_forecast_{h}d.csv"
                st.download_button(f"Download {labels[h]}", data=plot_df.to_csv(index=False).encode("utf-8"),
                                   file_name=csv_name, mime="text/csv", key=f"dlf_rice_{h}")
    else:
...
with tab2:
    ...
    if not files:
        ...
        for path in files:
            ...
            latest_actual = pd.to_numeric(df.iloc[:, -1], errors="coerce").dropna().iloc[-1] if not df.empty else None
            first_fore = outs[7]["mean"].iloc[0] if not outs[7].empty else float('nan')
            last_fore  = outs[365]["mean"].iloc[-1] if not outs[365].empty else float('nan')
